    ...
    internal static class WorkflowActions
    {
        internal static async Task Display(Guid guid)
        {
            ...
            while (true)
            {
                ...
                layout["Header"].Update(
                    new Rule($"[blue bold]{string.Format(Strings.GH_Wf_Title, Markup.Escape(GetProject(guid).GitHubName))}[/]")
                        .LeftJustified());
                    ...
                    .Start(ctx =>
                    {
                        ...
                        while (running)
                        {
                            ...
                            var table = new Table()
                                ...
                                .AddColumn($"[grey]#[/]")
                                .AddColumn($"[grey]{Strings.GH_Wf_Col_Workflow}[/]")
                                .AddColumn($"[grey]{Strings.GH_Wf_Col_Status}[/]")
                                .AddColumn($"[grey]{Strings.GH_Wf_Col_Event}[/]")
                                .AddColumn($"[grey]{Strings.GH_Wf_Col_Branch}[/]")
                                .AddColumn($"[grey]{Strings.GH_Wf_Col_Commit}[/]")
                                .AddColumn($"[grey]{Strings.GH_Wf_Col_When}[/]")
                                .Expand();
                            ...
                            for (var i = firstRow; i < lastRow; i++)
                            {
                                ...
                                if (i == selectedIndex)
                                    table.AddRow(
                                        "[Aqua]▸[/]",
                                        $"[Aqua]{run.RunNumber}[/]",
                                        $"[Aqua]{Markup.Escape(run.Name ?? Strings.Common_Unknown)}[/]",
                                        $"[Aqua]{state.Glyph} {state.Label}[/]",
                                        $"[Aqua]{Markup.Escape(run.Event ?? "")}[/]",
                                        $"[Aqua]{Markup.Escape(run.HeadBranch ?? "")}[/]",
                                        $"[Aqua]{ShortSha(run.HeadSha)}[/]",
                                        $"[Aqua]{GitHubActions.RelativeTime(run.UpdatedAt.ToLocalTime())}[/]");
                                else
                                    table.AddRow(
                                        "",
                                        ...
                                        $"[grey]{Markup.Escape(run.Event ?? "")}[/]",
                                        $"[Green3_1]{Markup.Escape(run.HeadBranch ?? "")}[/]",
                                        $"[DarkOrange3]{ShortSha(run.HeadSha)}[/]",
                                        $"[CadetBlue]{GitHubActions.RelativeTime(run.UpdatedAt.ToLocalTime())}[/]");
                            }

                            layout["List"].Update(visible.Count == 0
                                ? new Markup($"\n    [Red3_1]{Strings.GH_Wf_NoRunsFiltered}[/]")
                                : table);
                            ...
                            layout["Footer"].Update(new Panel(
                                new Grid()
                                    ...
                                    .Expand()
                                    .AddRow(
                                        $"[grey]{Strings.GH_Wf_Footer_List}" +
                                        $"{(workflows is { Count: > 0 } ? Strings.GH_Wf_Key_Dispatch : "")}" +
                                        $"{Strings.GH_Wf_Key_Refresh}{Strings.GH_Wf_Key_Browser}[/]",
                                        $"[grey]{string.Format(Strings.GH_Wf_Page, visible.Count == 0 ? 0 : currentPage + 1, visible.Count == 0 ? 0 : lastPage, visible.Count == 0 ? 0 : selectedIndex + 1, visible.Count)}[/]"))
                                .RoundedBorder()
                                .Expand()
                                .Padding(1, 0));
            ...
            while (true)
            {
                ...
                foreach (var job in jobs)
                {
                    ...
                    lines.Add($"[{jobState.Colour}]{jobState.Glyph}[/] [bold]{Markup.Escape(job.Name ?? "")}[/]   " +
                              $"[CadetBlue]{Elapsed(job.StartedAt, job.CompletedAt)}[/]");
                    ...
                    foreach (var step in job.Steps)
                    {
                        ...
                        lines.Add($"    [{stepState.Colour}]{stepState.Mark}[/] {Markup.Escape(step.Name ?? "")}   " +
                                  (step.Conclusion is "skipped" or "cancelled" or "neutral"
                                      ? $"[grey]{stepState.Label}[/]"
                                      : $"[CadetBlue]{Elapsed(step.StartedAt, step.CompletedAt)}[/]"));
                    }
                ...
                details.AddRow(
                    ...
                    $"[bold]{Strings.GH_Wf_Row_Branch}[/]", $"[Green3_1]{Markup.Escape(current.HeadBranch ?? "")}[/]");

                details.AddRow(
                    $"[bold]{Strings.GH_Wf_Row_Commit}[/]", $"[DarkOrange3]{ShortSha(current.HeadSha)}[/]",
                    $"[bold]{Strings.GH_Wf_Row_Actor}[/]", $"[SkyBlue1]{Markup.Escape(current.Actor?.Login ?? "")}[/]");

                details.AddRow(
                    $"[bold]{Strings.GH_Wf_Row_Started}[/]", $"[CadetBlue]{(current.StartedAt?.ToLocalTime().ToString("g") ?? "")}[/]",
                    $"[bold]{Strings.GH_Wf_Row_Duration}[/]", $"[CadetBlue]{Elapsed(current.StartedAt, current.UpdatedAt)}[/]");
                ...
                layout["Header"].Update(
                    new Rule($"[blue bold]{string.Format(Strings.GH_Wf_RunTitle, current.RunNumber, Markup.Escape(current.Name ?? ""))}[/]")
                        .LeftJustified());
                            ...
                            layout["Footer"].Update(new Panel(
                                new Grid()
                                    ...
                                    .AddRow(
                                        ...
                                        $"[grey]{Strings.GH_Wf_Key_FullLog}{Strings.GH_Wf_Key_Refresh}{Strings.GH_Wf_Key_Browser}[/]",
                                        $"[grey]{string.Format(Strings.GH_Wf_Jobs_Count, jobs.Count)}[/]"))
                                .RoundedBorder()
                                .Expand()
                                .Padding(1, 0));
        ...
        private static async Task ShowLog(Guid guid, GitHubWorkflowRun run)
        {
            ...
            var lines = logs
                ...
                .SelectMany(x => new[] { "", $"[Aqua bold]{Markup.Escape(x.Key)}[/]", "" }
            ...
            layout["Header"].Update(
                new Rule($"[blue bold]{string.Format(Strings.GH_Wf_LogTitle, run.RunNumber)}[/]").LeftJustified());
                    ...
                    while (running)
                    {
                        ...
                        layout["Footer"].Update(new Panel(
                            new Grid()
                                ...
                                .Expand()
                                .AddRow(
                                    $"[grey]{Strings.GH_Wf_Footer_Log}[/]",
                                    $"[grey]{string.Format(Strings.GH_Wf_LogLines, scroll + 1, Math.Min(scroll + height, lines.Count), lines.Count)}[/]"))
                            .RoundedBorder()
                            .Expand()
                            .Padding(1, 0));
        ...
        private static async Task DispatchWorkflow(Guid guid, List<GitHubWorkflow> workflows)
        {
            ...
            var choice = AnsiConsole.Prompt(
                new SelectionPrompt<GitHubWorkflow?>()
                    ...
                    .UseConverter(x => x == null
                        ...
                        : $"{Markup.Escape(x.Name ?? "")}   [grey]{Markup.Escape(x.Path ?? "")}[/]"));
            ...
            var branch = AnsiConsole.Prompt(
                new SelectionPrompt<string>()
                    ...
                    .AddChoices(branches)
                    .UseConverter(x => $"[Green3_1]{Markup.Escape(x)}[/]"));
            ...
            if (outcome.Error != null)
                ...
                UI.Success(string.Format(Strings.GH_Wf_Dispatched,
                    $"[Aqua]{Markup.Escape(choice.Name ?? "")}[/]",
                    $"[Green3_1]{Markup.Escape(branch)}[/]"), Strings.GH_Wf_Title_Short);
